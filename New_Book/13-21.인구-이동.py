...
def imig(r, c, union_number):
    # 현재 위치 (r, c)에서 연결된 모든 연합을 찾고,
    # visited 배열에 연합 정보를 기록한 수
    # 각 위치의 인구를 분배하는 함수
    # BFS
    q = deque([])
    q.append((r, c))
    ...
    while q:
        ...
        for i in range(4):
            ...
            if 0 <= nr < N and 0 <= nc < N \
                ...
                q.append((nr, nc))
                imig_nation_list.append((nr, nc))
                visited[nr][nc] = union_number
                union_pop_sum += board[nr][nc]
    
    # 인구 분배
    union_num = len(imig_nation_list)
    if union_num == 0:
        union_num = 1
    div_pop = union_pop_sum // union_num
    for r, c in imig_nation_list:
        board[r][c] = div_pop
